...
class Model(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        if self.use_transfer:
            ...
            if not self.condition:
                ...
                self.mixt_logits_fw = nn.ModuleList([nn.Sequential(
                    ...
                    nn.Linear(self.word_dim, config.num_mixt)
        ...
        if self.use_transfer:
            if self.gate_fuse == 0:
                self.rnn = EncoderRNN(config.char_hidden+self.word_dim*(1+self.att_cnt*2), config.hidden0, 1, True, True, 1-config.keep_prob0, False)
            elif self.gate_fuse == 1:
                self.gru_cell = nn.GRUCell(self.att_cnt * 2 * self.word_dim, self.word_dim)
                self.rnn = EncoderRNN(config.char_hidden+self.word_dim*2, config.hidden0, 1, True, True, 1-config.keep_prob0, False)
            elif self.gate_fuse == 2:
                self.gate_layer = GateLayer(self.word_dim * (self.att_cnt * 2 + 1), self.word_dim)
                self.rnn = EncoderRNN(config.char_hidden+self.word_dim*2, config.hidden0, 1, True, True, 1-config.keep_prob0, False)
            elif self.gate_fuse == 3:
                self.gate_layer = GateLayer(self.word_dim * (self.att_cnt * 2 + 1), self.word_dim * max(1, self.att_cnt // 2))
                self.rnn = EncoderRNN(config.char_hidden+self.word_dim*(1 + max(1, self.att_cnt // 2)), config.hidden0, 1, True, True, 1-config.keep_prob0, False)
            else:
                assert False
        elif self.uniform_graph:
            if self.gate_fuse == 0:
                self.rnn = EncoderRNN(config.char_hidden+self.word_dim*2, config.hidden0, 1, True, True, 1-config.keep_prob0, False)
            elif self.gate_fuse == 1:
                self.gru_cell = nn.GRUCell(self.word_dim, self.word_dim)
                self.rnn = EncoderRNN(config.char_hidden+self.word_dim*2, config.hidden0, 1, True, True, 1-config.keep_prob0, False)
            elif self.gate_fuse == 2:
                self.gate_layer = GateLayer(self.word_dim * 2, self.word_dim)
                self.rnn = EncoderRNN(config.char_hidden+self.word_dim*2, config.hidden0, 1, True, True, 1-config.keep_prob0, False)
            elif self.gate_fuse == 3:
                self.gate_layer = GateLayer(self.word_dim * 2, self.word_dim)
                self.rnn = EncoderRNN(config.char_hidden+self.word_dim*2, config.hidden0, 1, True, True, 1-config.keep_prob0, False)
        ...
        self.qc_att = BiAttention(config.hidden0*2, 1-config.keep_prob, square=config.square)
        self.linear_1 = nn.Sequential(
                nn.Linear(config.hidden0*8, config.hidden),
                nn.ReLU()
            )

        self.rnn_2 = EncoderRNN(config.hidden, config.hidden1, 1, False, True, 1-config.keep_prob, False)
        self.self_att = BiAttention(config.hidden1*2, 1-config.keep_prob, square=config.square)
    ...
    def forward(self, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, return_yp=False, pre_att=None, pre_att_q=None, elmo=None, elmo_q=None):
        ...
        if self.use_transfer or self.uniform_graph:
            if self.gate_fuse == 0:
                states_p = [context_word, context_ch]
                states_q = [ques_word, ques_ch]
            elif self.gate_fuse == 1:
                states_p, states_q = [], []
            elif self.gate_fuse == 2:
                states_p, states_q = [context_word], [ques_word]
            elif self.gate_fuse == 3:
                states_p, states_q = [context_word], [ques_word]
            else:
                assert False
            ...
            if self.uniform_graph:
                def _get_att(x, x_mask):
                    return (x * x_mask[:, :, None]).mean(dim=1, keepdim=True).expand_as(x)
                states_p.append(_get_att(context_word, context_mask))
                states_q.append(_get_att(ques_word, ques_mask))
            else:
                graph_layers = pre_att.size(1) // 2

                if self.gate_fuse < 3:
                    def _resize(x, input_len):
                        return x.permute(0, 3, 4, 1, 2).contiguous().view(bsz, input_len, input_len, -1)
                    pre_att_fw = _resize(pre_att[:, :graph_layers], para_size)
                    pre_att_bw = _resize(pre_att[:, graph_layers:], para_size)
                    pre_att_q_fw = _resize(pre_att_q[:, :graph_layers], ques_size)
                    pre_att_q_bw = _resize(pre_att_q[:, graph_layers:], ques_size)
                    ...
                    for i in range(self.att_cnt):
                        if not self.condition:
                            ...
                            mixt_weights_fw = F.softmax(self.mixt_logits_fw[i](context_word), dim=-1)
                            mixt_weights_bw = F.softmax(self.mixt_logits_bw[i](context_word), dim=-1)
                            mixt_weights_q_fw = F.softmax(self.mixt_logits_fw[i](ques_word), dim=-1)
                            mixt_weights_q_bw = F.softmax(self.mixt_logits_bw[i](ques_word), dim=-1)

                        pre_att_t_fw = torch.matmul(pre_att_fw, mixt_weights_fw.unsqueeze(-1)).squeeze(-1)
                        pre_att_t_bw = torch.matmul(pre_att_bw, mixt_weights_bw.unsqueeze(-1)).squeeze(-1)
                        pre_att_q_t_fw = torch.matmul(pre_att_q_fw, mixt_weights_q_fw.unsqueeze(-1)).squeeze(-1)
                        pre_att_q_t_bw = torch.matmul(pre_att_q_bw, mixt_weights_q_bw.unsqueeze(-1)).squeeze(-1)

                        pre_att_t_fw = torch.matmul(pre_att_t_fw, context_word)
                        pre_att_t_bw = torch.matmul(pre_att_t_bw, context_word)
                        pre_att_q_t_fw = torch.matmul(pre_att_q_t_fw, ques_word)
                        pre_att_q_t_bw = torch.matmul(pre_att_q_t_bw, ques_word)
            ...
            if self.gate_fuse == 0:
                context_output = context_merge
                ques_output = ques_merge
            elif self.gate_fuse == 1:
                context_att = self.gru_cell(self.dropout(context_merge).view(bsz*para_size, -1), self.dropout(context_word).view(bsz*para_size, -1)).view(bsz, para_size, -1)
                ques_att = self.gru_cell(self.dropout(ques_merge).view(bsz*ques_size, -1), self.dropout(ques_word).view(bsz*ques_size, -1)).view(bsz, ques_size, -1)
                context_output = torch.cat([context_word, context_ch, context_att], dim=2)
                ques_output = torch.cat([ques_word, ques_ch, ques_att], dim=2)
            elif self.gate_fuse == 2:
...
class BiAttention(nn.Module):
    def __init__(self, input_size, dropout, square=False):
        super().__init__()
        self.dropout = LockedDropout(dropout)
    ...
    def forward(self, input, memory, mask):
        ...
        if not self.square:
            ...
            att = att * mask[:,None]
            att.pow_(2)
            weight_one = att.div(att.sum(dim=-1, keepdim=True) + 1e-16)
        ...
        if not self.square:
            ...
            weight_two = att.max(dim=-1)[0]
            weight_two = weight_two.div(weight_two.sum(dim=-1, keepdim=True) + 1e-16).view(bsz, 1, input_len)
